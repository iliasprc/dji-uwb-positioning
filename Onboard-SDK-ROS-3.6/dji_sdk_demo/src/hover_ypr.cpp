...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber uwbPosition = nh.subscribe("uwb_pos", 20, &uwb_position_callback);
  // Publish the control signal
  ctrlRollPitchYawHeightPub = nh.advertise<sensor_msgs::Joy>("dji_sdk/flight_control_setpoint_rollpitch_yawrate_zposition", 10);
  // Basic services
  ...
  if (elapsed_time > ros::Duration(0.02)) {
    start_time = ros::Time::now();
      ...
      //! First arbitrary target
        ...
        setTarget(initial_pos.x, initial_pos.y, 2, 0);
...
void local_position_ctrl(double &xCmd, double &yCmd, double &zCmd)
{
  ...
  x_error = target_offset_x - uwb_pos.x;
  y_error = target_offset_y - uwb_pos.y;
  zCmd = target_offset_z;
  ix_error=ix_error+x_error;
  iy_error=iy_error+y_error;
  float pitchd=kp*x_error+ki*ix_error;
  float rolld=kp*y_error+ki*iy_error;
  
  sensor_msgs::Joy controlmsg;
  controlmsg.axes.push_back(rolld);
  controlmsg.axes.push_back(pitchd);
  controlmsg.axes.push_back(zCmd);
  controlmsg.axes.push_back(0);
  ctrlPosYawPub.publish(controlmsg);
  //
